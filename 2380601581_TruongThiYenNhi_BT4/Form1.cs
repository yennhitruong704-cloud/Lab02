...
namespace Lab02_02
{
    public partial class Form1 : Form
    {
        ...
        private void AddSampleStudents()
        {
            ...
            {
                // { MSSV, Họ Tên, Giới Tính (Nam/Nữ), Điểm TB, Khoa }
                { "1001", "Trần Văn A", "Nam", "7.5", "CNTT" },
                { "1002", "Nguyễn Thị B", "Nữ", "8.2", "NNA" },
                { "1003", "Lê Văn C", "Nam", "9.0", "CNTT" },
                { "1004", "Phạm Thị D", "Nữ", "9.0", "CNTT" },
                { "1005", "Hoàng Văn E", "Nam", "6.7", "NNA" },
                { "1006", "Ngô Thị F", "Nữ", "7.1", "QTKD" },
                { "1007", "Cao Văn G", "Nam", "5.0", "CNTT" },
                { "1008", "Võ Thị H", "Nữ", "8.8", "QTKD" },
                { "1009", "Cao Văn I", "Nam", "6.0", "QTKD" },
                { "1010", "Lê Thị K", "Nữ", "9.5", "NNA" }
            };
            for (int i = 0; i < sampleData.GetLength(0); i++)
            {
                int rowIndex = dgvStudent.Rows.Add();
                dgvStudent.Rows[rowIndex].Cells[colStudentID.Index].Value = sampleData[i, 0];
                dgvStudent.Rows[rowIndex].Cells[colFullName.Index].Value = sampleData[i, 1];
                dgvStudent.Rows[rowIndex].Cells[colGender.Index].Value = sampleData[i, 2];
                dgvStudent.Rows[rowIndex].Cells[colAverageScore.Index].Value = sampleData[i, 3];
                dgvStudent.Rows[rowIndex].Cells[colFaculty.Index].Value = sampleData[i, 4];
            }
        }
